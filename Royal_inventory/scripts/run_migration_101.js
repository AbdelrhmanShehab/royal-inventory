'use strict';
/**
 * Run Migration 101 — Performance Indexes on Laundry Module Tables
 */
...
async function run() {
  ...
  const sql  = fs.readFileSync(path.join(__dirname, '../migrations/101_laundry_indexes_optimizations.sql'), 'utf8');
...
run().catch(e => { console.error('❌ Migration 101 failed:', e.message); process.exit(1); });
